...
void Viewport3D::zoomToFit(const SceneMesh& mesh) {
    if (mesh.vertices.empty()) return;

    // Compute bounding box
    glm::vec3 lo(mesh.vertices[0].px, mesh.vertices[0].py, mesh.vertices[0].pz);
    glm::vec3 hi = lo;
    for (const auto& v : mesh.vertices) {
        lo.x = std::min(lo.x, v.px); lo.y = std::min(lo.y, v.py); lo.z = std::min(lo.z, v.pz);
        hi.x = std::max(hi.x, v.px); hi.y = std::max(hi.y, v.py); hi.z = std::max(hi.z, v.pz);
    }

    camTarget_ = (lo + hi) * 0.5f;
    float diag = glm::length(hi - lo);
    // Place camera so the object fills roughly 60% of view at 45° FOV
    camDist_ = diag / (2.0f * std::tan(glm::radians(45.0f) * 0.5f)) * 1.2f;
    camDist_ = glm::clamp(camDist_, 1.0f, 5000.0f);
    camAz_  = 45.0f;
    camEl_  = 30.0f;
}
